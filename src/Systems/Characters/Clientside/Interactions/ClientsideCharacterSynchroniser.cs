...
public sealed class ClientsideCharacterSynchroniser : ICharacterSynchroniser
{
    public ClientsideCharacterSynchroniser(ICharacterProvider provider)
    {
        Provider = provider;
    ...
    private void SendPacket(SyncType syncType, byte[] packet)
    {
        ...
        {
            throw new NotSupportedException($"SyncType.{syncType} is not supported on the clientside. Only SyncType.Exclude is supported.");
        }
    }
}
