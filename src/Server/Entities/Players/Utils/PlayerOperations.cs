...
{
    public static Action<PlayerEntity, string> GlobalKickOperation { get; set; } = static (player, reason) =>
    {
        NetMessage.SendData(2, player.Index, -1, NetworkText.FromLiteral(reason));
    };
    public Action<PlayerEntity, string>? PersonalKickOperation { get; set; }

    public static Action<PlayerEntity, byte[]> GlobalSendBytesOperation { get; set; } = static (player, bytes) =>
    {
    ...
    public Action<PlayerEntity, byte[]>? PersonalSendBytesOperation { get; set; }
}
