...
internal sealed class TileFix : WorldGen
{
    internal static void AttachHooks()
    {
        ...
        On.Terraria.WorldGen.GrowEpicTree += OnGrowEpicTree;
    }

    private static bool OnGrowEpicTree(On.Terraria.WorldGen.orig_GrowEpicTree orig, int i, int y)
    {
		int j;
		for (j = y; Main.tile[i, j].type == 20 && j < Main.maxTilesY - 2; j++)
		{
		}
