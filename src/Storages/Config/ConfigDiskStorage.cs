...
internal static class ConfigDiskStorage
{
    ...
    {
        return Path.Combine(AmethystSession.Profile.SavePath, name);
    }

    internal static void ReadOrCreate<T>(string name, ref T def)
    {
        if (File.Exists(GetPath($"{name}.json")) == false)
        {
            Write(name, (object)def!);
            return;
        }
    ...
    {
        var json = File.ReadAllText(GetPath($"{name}.json"));
